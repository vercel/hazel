...
// Utilities
const { checkPlatform, checkArch } = require('./platform')
...
module.exports = class Cache {
  ...
  async refreshCache() {
    ...
    for (const asset of release.assets) {
      ...
      const arch = checkArch(name)

      if (!platform || !arch) {
        continue
      }

      this.latest.platforms[platform] = this.latest.platforms[platform] || {}

      this.latest.platforms[platform][arch] = {
        name,
        api_url: url,
