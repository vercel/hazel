...
exports.download = (req, res) => {
  ...
  res.writeHead(302, {
    Location: platforms[platform].x64.url
  })
...
exports.downloadPlatform = (req, res) => {
  ...
  const package = arch
  ? latest.platforms[platform][arch]
  : latest.platforms[platform].x64

  if (!package || !package.url) {
    send(res, 404, 'No download available for your architecture')
    return
  }

  if (TOKEN && typeof TOKEN === 'string' && TOKEN.length > 0) {
    proxyPrivateDownload(package, req, res)
    return
  }
...
exports.update = (req, res) => {
  const { platform: platformName, version, arch } = req.params
  ...
  // By default, x64 architecture is used
  const package = arch
    ? latest.platforms[platform][arch]
    : latest.platforms[platform].x64
