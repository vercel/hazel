...
module.exports = ({ cache, config }) => {
  ...
  exports.update = async (req, res) => {
    ...
    if (!latest.platforms || !latest.platforms[platform]) {
      console.log(`Platforms is null or ${platform} is missing; returning 204`)
      res.statusCode = 204
      res.end()
    ...
    if (compare(latest.version, version) !== 0) {
      ...
      console.log(
        `Update is available for ${platform} ${version}; returning 200`
      )
    ...
    console.log(`No update available for ${platform} ${version}; returning 204`)
  ...
  exports.overview = async (req, res) => {
    ...
    // be shown on the overview page and rename some
    // to more commonly-known labels
    const overviewPlatforms = {}
    const deletes = ['dmg', 'nupkg']
    const renames = {
      exe: 'Windows',
      darwin: 'macOS',
      linux: 'Linux'
    ...
    if (latest.platforms) {
      Object.keys(latest.platforms).forEach(platform => {
        if (!deletes.includes(platform)) {
          if (Object.keys(renames).includes(platform)) {
            overviewPlatforms[renames[platform]] = latest.platforms[platform]
            if (shouldProxyPrivateDownload) {
              overviewPlatforms[
                renames[platform]
              ].url = `${hURL}/download/${platform}`
            }
          } else {
            overviewPlatforms[platform] = latest.platforms[platform]
            if (shouldProxyPrivateDownload) {
              overviewPlatforms[platform].url = `${hURL}/download/${platform}`
            }
          }
        }
      })
