...
module.exports = ({ cache, config }) => {
  ...
  exports.downloadPlatform = async (req, res) => {
    ...
    let { platform, arch = 'x64' } = req.params
    ...
    // Use the specified arch or fall back to x64
    const pkg = latest.platforms[platform][arch]
  ...
  exports.update = async (req, res) => {
    const { platform: platformName, version, arch = 'x64' } = req.params
    ...
    if (compare(latest.version, version) !== 0) {
      ...
      send(res, 200, {
        ...
        url: shouldProxyPrivateDownload
          ? `${url}/download/${platformName}/${arch}?update=true`
          : pkg.url
      })
